...
const webglTrailTriangles = [];
let webglTrailSegmentCount = 0;

geometryEffect.webglBloomRenderer =
{
  available: true,
  stats:
  {
  ...
  addTrailTriangle(first, second, third, colors, opacity)
  {
    webglTrailTriangles.push(
      {
...
geometryEffect._renderWebGL2Bloom(1);

const webglTrailVertices = webglTrailTriangles.flatMap(
  ({ points }) => points,
);
const webglHalfWidth = UNITY_FX_TOUCH.trail.geometryWidth * 0.5 *
  UNITY_FX_TOUCH.bloom.trailCoverageScale;
const webglEndCap = { x: 170, y: 170 + webglHalfWidth };
const hasWebGLTrailVertex = (expected) => webglTrailVertices.some((point) =>
  Math.abs(point.x - expected.x) < 0.000001 &&
    Math.abs(point.y - expected.y) < 0.000001);

assert(
  webglTrailSegmentCount === 0 &&
    webglTrailTriangles.length > canvasTrailBandCount * 4 &&
    hasWebGLTrailVertex(webglEndCap) &&
    rightAngleOuterArc.every(([x, y]) =>
      hasWebGLTrailVertex({ x, y })),
  'WebGL2 Bloom 复用圆角、端帽和有限 miter 的 TrailRenderer 网格',
);
