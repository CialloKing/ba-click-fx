...
- Canvas、Native、Legacy 和完整 WebGL2 共享 TrailRenderer 折点、内角交点、转角扇面与 `numCapVertices = 1` 首尾端帽；WebGL2 按弧长插值 Stretch 能量和横向轮廓
- Native 与 Legacy 增加阈值驱动的圆环、光盘、碎片和拖尾辉光；拖尾改为一次局部离屏模糊，并复用 Unity 资源参数和生命周期
- `pointerCancel()` 强制清理活动指针、当前位置和当前轨迹；`pointerUp()` 继续让已有拖尾自然消失，`trailAlways` 无可见内容时停止 RAF 并由下一次移动唤醒
