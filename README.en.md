...
`outputCompositing: 'scene'` preserves Unity's direct additive RGB semantics for a known Scene render target. `'transparent-overlay'` targets transparent desktop windows such as WebView2 and Electron: HDR RGB still drives Bloom, while geometry Coverage, lifetime alpha, and `opacity` provide an upper bound for final alpha. With an unknown background, visible RGB energy caps that alpha again, preventing both a high-HDR centre disk from fully occluding the desktop and a low-energy trail tail from darkening a light background. It does not alter Bloom thresholds or emission strength.
